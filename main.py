...
def main(config):
    ...
    # Define optimizer
    optimizer = torch.optim.Adam(net.parameters(), lr=learning_rate, weight_decay=1e-05)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=config.lr_steps, gamma=0.1)
    print(optimizer)
    
    # Load model
    log_dir = f"./logs/{config.expt_name}/"
    if os.path.exists(log_dir) and os.path.isfile(log_dir+"best_val_checkpoint.tar") and config.resume:
        if torch.cuda.is_available():
    ...
    epoch_bar = master_bar(range(1, max_epochs + 1))

    if config.val:
        print('Validating..')
        test_time, test_loss, test_err_edges, test_err_tour, test_err_tsp, test_pred_tour_len, test_gt_tour_len = test(net, config, epoch_bar, mode='test')
        print(f'Val results: loss {test_loss} test_err_tsp {test_err_tsp} test_pred_tour_len {test_pred_tour_len} test_gt_tour_len {test_gt_tour_len}')
        return
    ...
    for epoch in epoch_bar:
        ...
        # Train
        train_time, train_loss, train_err_edges, train_err_tour, train_err_tsp, train_pred_tour_len, train_gt_tour_len = train_one_epoch(net, optimizer, config, epoch_bar)
        epoch_bar.write('t: ' + metrics_to_str(epoch, train_time, get_lr(optimizer), train_loss, train_err_edges, train_err_tour, train_err_tsp, train_pred_tour_len, train_gt_tour_len))
        ...
        if epoch % val_every == 0 or epoch == max_epochs:
            # Validate
            val_time, val_loss, val_err_edges, val_err_tour, val_err_tsp, val_pred_tour_len, val_gt_tour_len = test(net, config, epoch_bar, mode='val')
            epoch_bar.write('v: ' + metrics_to_str(epoch, val_time, get_lr(optimizer), val_loss, val_err_edges, val_err_tour, val_err_tsp, val_pred_tour_len, val_gt_tour_len))
        ...
        if epoch % test_every == 0 or epoch == max_epochs:
            # Test
            test_time, test_loss, test_err_edges, test_err_tour, test_err_tsp, test_pred_tour_len, test_gt_tour_len = test(net, config, epoch_bar, mode='test')
            epoch_bar.write('T: ' + metrics_to_str(epoch, test_time, get_lr(optimizer), test_loss, test_err_edges, test_err_tour, test_err_tsp, test_pred_tour_len, test_gt_tour_len))
        ...
        if epoch != 0 and (epoch % 250 == 0 or epoch == max_epochs):
            torch.save({
                'epoch': epoch,
                'model_state_dict': net.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
